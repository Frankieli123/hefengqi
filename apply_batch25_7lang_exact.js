...
async function run() {
  const specs = JSON.parse(fs.readFileSync("/mnt/vscode/hefengqi/batch25_kvm_specs.json", "utf8"));
  const transData = JSON.parse(fs.readFileSync("/mnt/vscode/hefengqi/batch25_translations_5langs.json", "utf8"));
  console.log("=== 开始精确按 normalizedId 绑定应用 Batch 25 的 5 语深度翻译 ===\n");
  ...
  console.log("\n🎉 全部 10 款 KVM 产品的 5 语深度本地化翻译绑定完成！");
}
