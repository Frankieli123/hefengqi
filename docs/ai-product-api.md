...
本 API 专为外部大语言模型（AI 智能体）、数据采集与自动化工作流设计，用于自动向合丰旗平台上传产品基本信息、三语说明文档（定义/这是什么/解决问题/核心优势/应用领域/FAQ）、技术规格参数及产品手册/图纸。
...
| **批量/单件上传** | `POST` | `/api/admin/products` | 上传产品、三语说明、规格参数及关联素材 |
...
### 3.2 上传产品及三语说明：`POST /api/admin/products`
...
#### 特性与容错机制：
1. **自动补齐多语言**：如果 AI 仅提供了中文（或顶层扁平字段），系统会自动为缺少语言生成合规的占位草稿，确保符合系统多语言架构。
2. **定义字数校验保证**：合丰旗发布门禁要求三语直接定义不少于 40 字符，接口在必要时会自动协助补足完整说明，防止被发布硬门禁阻断。
...
产品详情页的黑色关键参数区最多显示 6 项。参数数组中的 `featured: true` 表示展示该项，`featureOrder` 控制顺序（`0`–`5`），`displayLabel` 用于单独修改该产品上的展示标题，不会改动同分类其他产品的参数名称。`displayLabel` 可填写单个字符串，也可填写 `{ "zh": "…", "en": "…", "ru": "…" }`。未提交任何 `featured` 配置的旧产品默认展示排序最前的 6 项，后台保存一次“关键参数展示”后即以人工选择为准。
