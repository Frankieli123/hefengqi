...
const VIEW_DESC_5LANG = [
  ...
  { ru: "Внутренняя компоновка и автоматические выключатели", fr: "Disposition interne et disjoncteurs", de: "Innenaufbau und Leistungsschalter", es: "Disposición interna y disyuntores de protección", ar: "التصميم الداخلي وقواطع الدائرة الكهربائية" }
];

async function run() {
  const transData = JSON.parse(fs.readFileSync("/mnt/vscode/hefengqi/batch23_translations_5langs.json", "utf8"));
  console.log(`=== 开始全量应用 Batch 23 对应 5 种外语 (ru, fr, de, es, ar) 深度本地化翻译 ===\n`);

  for (const [model, transByLoc] of Object.entries(transData)) {
    ...
    console.log(`\nUpdating 5 languages for: [${product.model}] (ID: ${product.id})`);

    // 1. Update ProductTranslation & FAQs for ru, fr, de, es, ar
    for (const loc of LOCALES) {
      const tData = transByLoc[loc];
      if (!tData) continue;
    ...
    console.log(`🟢 [${model}] 5语本地化深度写入完成！`);
  }

  console.log("\n🎉 全部 20 款产品的 5 种外语本地化已 100% 写入数据库！");
}
