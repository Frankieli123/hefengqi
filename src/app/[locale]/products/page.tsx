...
export default async function ProductsPage({ params, searchParams }: Props) {
  ...
  return <main id="main-content"><JsonLd data={[breadcrumbSchema([{ name: common("home"), path: `/${locale}` }, { name: t("title"), path: `/${locale}/products` }]), collectionPageSchema(locale, "/products", t("title"), t("description"), products.map((product) => ({ name: product.name, path: `/products/${product.slug}` })))]} /><section className="bg-ink text-ink-foreground"><div className="page-shell py-14"><h1 className="text-3xl font-semibold leading-tight md:text-5xl">{t("title")}</h1><p className="mt-5 max-w-3xl text-base leading-7 text-ink-muted">{t("description")}</p></div></section><Catalog locale={locale} products={products} categories={categories} labels={labels} query={query} /></main>;
}
