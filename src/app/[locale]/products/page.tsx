...
export default async function ProductsPage({ params, searchParams }: Props) {
  ...
  return <main id="main-content"><JsonLd data={[breadcrumbSchema([{ name: common("home"), path: `/${locale}` }, { name: t("title"), path: `/${locale}/products` }]), collectionPageSchema(locale, "/products", t("title"), t("description"))]} /><section className="bg-ink text-ink-foreground"><div className="page-shell py-10 md:py-14"><h1 className="heading-underlined heading-underlined-left text-3xl font-semibold leading-tight md:text-5xl">{t("title")}</h1><p className="mt-4 md:mt-5 max-w-3xl text-sm md:text-base leading-relaxed md:leading-7 text-ink-muted">{t("description")}</p></div></section><Catalog locale={locale} products={catalog.products} categories={categories} labels={labels} query={query} currentPage={catalog.currentPage} pageCount={catalog.pageCount} totalCount={catalog.totalCount} /></main>;
}
