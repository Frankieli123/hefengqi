...
import { getCategories, getCategoryAlternatePaths, getProducts, getSlugRedirect } from "@/lib/content-repository";
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return localizedMetadata(locale, `/products/category/${slug.join("/")}`, category.seoTitle ?? category.name, category.seoDescription ?? category.description, Boolean(query.q || query.compare), alternates);
}
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  return <main id="main-content"><JsonLd data={[breadcrumbSchema([{ name: common("home"), path: `/${locale}` }, { name: t("title"), path: `/${locale}/products` }, { name: category.name, path: `/${locale}/products/category/${slug.join("/")}` }]), collectionPageSchema(locale, `/products/category/${category.path}`, category.name, category.description)]} /><section className="bg-ink text-ink-foreground"><div className="page-shell py-14"><span className="eyebrow">Catalog / {category.key}</span><h1 className="mt-6 text-3xl font-semibold leading-tight md:text-5xl">{category.name}</h1><p className="mt-5 max-w-3xl text-base leading-7 text-ink-muted">{category.description}</p></div></section><Catalog locale={locale} products={products} categories={categories} labels={labels} query={query} currentCategory={category.key} currentCategoryPath={category.path} /></main>;
}
