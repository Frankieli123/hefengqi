...
export default async function Page({ searchParams }: { searchParams: Promise<{ q?: string; error?: string; updated?: string }> }) {
  ...
  return <div className="flex flex-col gap-6 p-5 md:p-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold">产品分类</h1><p className="mt-2 text-sm text-muted-foreground">管理三级目录、七语名称、展示顺序与发布状态。已发布分类的修改会同步到官网。</p></div><Button nativeButton={false} render={<a href="/admin/categories/new" />}><PlusIcon data-icon="inline-start" />新建分类</Button></div>
