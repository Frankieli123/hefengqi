...
export function SupportDevice({ locale, product, categoryName, articles }: { locale: Locale; product: ProductView; categoryName: string; articles: EditorialItem[] }) {
  ...
  return <main id="main-content" className="support-page">
    <div className="page-shell support-breadcrumb"><Breadcrumb><BreadcrumbList>
      ...
      <BreadcrumbItem><BreadcrumbLink render={<Link locale={locale} href={supportHref({ brand: product.brand.toLowerCase() })} />}>{formatBrandName(product.brand)}</BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator />
      <BreadcrumbItem><BreadcrumbPage>{product.model}</BreadcrumbPage></BreadcrumbItem>
    </BreadcrumbList></Breadcrumb></div>
    <header className="support-detail-header"><div className="page-shell"><p className="support-meta">{formatBrandName(product.brand)} / {categoryName}</p><h1>{product.model}{" "}<span>{copy.detailSuffix}</span></h1><p className="support-detail-description">{product.directDefinition}</p></div></header>
    <div className="page-shell support-detail-layout">
      ...
      <div className="support-detail-body">
        <section id="equipment" aria-labelledby="equipment-title"><h2 id="equipment-title">{copy.overview}</h2><div className="support-equipment-overview"><div>
          <dl className="support-equipment-facts"><div><dt>{copy.brand}</dt><dd>{formatBrandName(product.brand)}</dd></div><div><dt>{copy.model}</dt><dd>{product.model}</dd></div><div><dt>{copy.category}</dt><dd>{categoryName}</dd></div><div><dt>{copy.version}</dt><dd>{copy.versionUnknown}</dd></div></dl>
