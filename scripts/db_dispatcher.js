...
async function poll() {
  try {
    ...
    for (const msg of newMessages) {
      ...
      console.log(`[${new Date().toISOString()}] New message detected: ${msg.body}`);
      postWebhook({
    ...
    for (const inq of newInquiries) {
      ...
      console.log(`[${new Date().toISOString()}] New inquiry detected: ${inq.referenceId}`);
      postWebhook({
...
async function main() {
  ...
  console.log(`DB Event Dispatcher started. Watching from msg: ${lastMessageTime.toISOString()}, inq: ${lastInquiryTime.toISOString()}`);
